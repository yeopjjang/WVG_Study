...
# Electron Selection
Electron_mask = (Electron.PT > 15) & (np.abs(Electron.Eta) < 2.5)
...
# Photon Selection
Photon_vari_mask = (Photon.PT > 20) & (np.abs(Photon.Eta) < 1.442) | ((np.abs(Photon.Eta) > 1.566) & (np.abs(Photon.Eta < 2.5)))
...
def make_TLV(eee):
	...
	TLV = TLorentzVectorArray.from_ptetaphim(eee.PT, eee.Eta, eee.Phi, eee.T)
	return TLV
...
# Electron PT cut
Elept_mask = (Triple_eee.lep1.PT >= 25) & (Triple_eee.lep2.PT >= 15) & (Triple_eee.lep3.PT >= 15)

Triple_eee.lep1 = Triple_eee.lep1[Elept_mask]
Triple_eee.lep2 = Triple_eee.lep2[Elept_mask]
Triple_eee.lep3 = Triple_eee.lep3[Elept_mask]

Enum_mask = (ak.num(Triple_eee.lep1.PT > 0)) & (ak.num(Triple_eee.lep2.PT > 0)) & (ak.num(Triple_eee.lep3.PT > 0))

Triple_eee.lep1 = Triple_eee.lep1[Enum_mask]
Triple_eee.lep2 = Triple_eee.lep2[Enum_mask]
Triple_eee.lep3 = Triple_eee.lep3[Enum_mask]
eee_Photon = eee_Photon[Enum_mask]
eee_MET = eee_MET[Enum_mask]

lepton1 = make_TLV(Triple_eee.lep1)
lepton2 = make_TLV(Triple_eee.lep2)
lepton3 = make_TLV(Triple_eee.lep3)
...
leading_electron = make_TLV(Triple_eee.lep1)
subleading_electron = make_TLV(Triple_eee.lep2)
third_electron = make_TLV(Triple_eee.lep3)

test = leading_electron + subleading_electron
...
zmass_window = (np.abs(dilep.mass - 91.1876) < 15)
...
#xsec
#genEvt
#process_name = # signal 0 others 1,2,3....
